...
if test:
    X_train = pd.read_csv(inputpath+'train.csv', nrows=100000, parse_dates=['click_time'])
else:
    X_train = pd.read_csv(inputpath+'train.csv', skiprows=range(1,144903891), nrows=40000000, parse_dates=['click_time'])

print(X_train.shape)

print('Extracting time info..')
...
print('Creating confidence rate for is_attributed...')
...
print('Click on app add before and after...')
HISTORY_CLICKS = {
...
print(ft)
ft.to_csv(outputpath + 'feature_importance_result')
plt.figure()
ft[['feature','gain']].plot(kind='barh', x='feature', y='gain', legend=False, figsize=(10, 20))
plt.gcf().savefig(outputpath+ 'features_importance.png')
