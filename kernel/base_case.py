...
# kernel address: https://www.kaggle.com/bk0000/non-blending-lightgbm-model-lb-0-977?scriptVersionId=3224614
# 疑问：
# 1. nextClick_shift 似乎没用

# Get 0.9787 with default kernel settings, but replacing NA values with the mean :)
...
def DO(train_frm,train_to, test_nrows, fileno):
    ...
    ### 有问题 需要解决
    predictors.extend(['ip', 'app','device','os', 'channel', 'hour'])
    ...
    if not debug:
        print("writing...")
        sub.to_csv('sub_it{}.csv.gz'.format(str(fileno)),index=False,compression='gzip')
    print("done...")
    return sub
...
sub=DO(frm,to,test_nrows, 'base_case')
