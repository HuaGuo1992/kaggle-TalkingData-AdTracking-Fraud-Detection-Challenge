...
def DO(train_frm,train_to, test_nrows, groups, rategroup, fileno, initial_cols=['ip', 'app','device','os', 'channel', 'hour']):
    ...
    params = {
        'learning_rate': 0.807161719126812,
        #'is_unbalance': 'true', # replaced with scale_pos_weight argument
        'num_leaves': 100,  # 2^max_depth - 1
        'max_depth': 50,  # -1 means no limit
        'min_child_samples': 168,  # Minimum number of data need in a child(min_data_in_leaf)
        'max_bin': 132,  # Number of bucketed bin for feature values
